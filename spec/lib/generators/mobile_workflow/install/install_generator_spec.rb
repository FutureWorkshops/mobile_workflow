  ...
  module Generators
    describe InstallGenerator, type: :generator do
      describe '#generate_controllers_and_routes' do
        let(:open_api_spec_mock) do
          instance_double(::MobileWorkflow::OpenApiSpec::Parser, controller_name_to_actions: controller_name_to_actions)
        end
        ...
        context 'no properties, index and show' do
          let(:controller_name_to_actions) { { 'Cities' => %w[index show] } }
          let(:model_name_to_properties) { {} }
        ...
        context 'two properties' do
          ...
          let(:controller_route) { 'resources :Cities, only: [:index]' }
          let(:root_route) { "root to: 'admin/Cities#index'" }
          let(:controller_generate) do
            'mobile_workflow:controller City --actions index --attributes name:string population:integer'
          end
        ...
        context 's3 storage' do
          ...
          let(:root_route) { "root to: 'admin/Cities#index'" }
          let(:controller_generate) do
            'mobile_workflow:controller City --actions index --attributes name:string population:integer --s3-storage'
          end
        ...
        context 'doorkeeper_oauth' do
          ...
          let(:root_route) { "root to: 'admin/Cities#index'" }
          let(:controller_generate) do
            'mobile_workflow:controller City --actions index --attributes name:string population:integer --doorkeeper-oauth'
          end
