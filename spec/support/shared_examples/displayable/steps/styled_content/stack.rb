shared_examples_for 'styled content stack' do |param|
  ...
  describe '#mw_stack_list_item' do
    context 'attachment', if: param[:active_storage_enabled] do
      let(:result) { subject.mw_stack_list_item(text: 'John Doe', detail_text: 'Participant', preview_url: preview_url) }
  ...
  describe '#mw_stack_button' do
    ...
    context 'system links' do
      let(:result) { subject.mw_stack_button(label: 'Donate', apple_system_url: 'https://test.com/donate', android_deep_link: 'https://test.com/donate', on_success: :none) }
    ...
    context 'link id' do
      let(:result) { subject.mw_stack_button(label: 'Import', link_id: '9e5ad548-dd58-4f9b-adbe-99c59049398', on_success: :reload, style: :outline) }
    ...
    context 'modal workflow' do
      let(:result) { subject.mw_stack_button(label: 'Import', modal_workflow_name: 'Import Activity', on_success: :reload, style: :outline) }
    ...
    context 'share text and share image url' do
      let(:result) { subject.mw_stack_button(label: 'Share Challenge', share_text: 'Challenge completed!', share_image_url: 'https://test.com/image', on_success: :none) }
